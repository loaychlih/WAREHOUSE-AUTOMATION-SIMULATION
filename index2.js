...
function main()
{
    document.getElementById("Click").onclick = function makeRows(rows, cols) {
        ...
        const rows = parseInt(inputSize.value);
        const cols = parseInt(inputSize.value);
    ...
    const nbrecaseinitiale = rows;
    const nbre_kiva = parseInt(inputKivas.value);
    ...
    function task(i,cpt) {
            ...
            for(let i=0;i<nbre_kiva;i++){
                    ...
                    while( ordcourt(indexespacket[i])!=rows-i-1){indexespacket[i]=kivas[i].indexpackets()
                        ordpacket[i] =  ordcourt(indexespacket[i])
                    }
                    
                
                    while(abspacket[i]==0 && indexesmagasinier[i]==indexespacket[i])
                    {
                    ...
                    //partie condition 
                     j=0
                    while(j<i){
                        while(ordpacket[i]==ordpacket[j] ||indexesmagasinier[j]==indexespacket[i]||abscourt(indexespacket[i])==0||ordcourt(indexespacket[i])!=rows-i-1){
                                    indexespacket[i]=randomplace(0,(rows-1)*(rows+1))
                                    ordpacket[i] =  ordcourt(indexespacket[i])
                                    abspacket[i] = abscourt(indexespacket[i])
                                    j=0
